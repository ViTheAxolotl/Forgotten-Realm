...
function init()
{
    ...
    document.addEventListener("keydown", (ev) => {key = ev.key.slice(ev.key.indexOf("w")).toLowerCase(); console.log(key); let keyValues = ["left", "right", "down", "up"]; if(keyValues.includes(key) && ev.ctrlKey) {handleArrow();}});
    setMainVaribles();
}
